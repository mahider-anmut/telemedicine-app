import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryBGColor = Color(0xFFFFFFFF);

  static const Color primaryColor = Color(0xff407CE2);
  static const Color primaryLightColor = Color(0xff3498DB);




  static const Color secondaryColor = Color(0xFF3E3E49);

  static const Color titleTextColor = Color(0xFF1E232C);
  static const Color mainTextColor = Color(0xFF6A707C);
  static const Color mainTextAltColor = Color(0xFFFFFFFF);

  static const Color mainIconColor = Color(0xFFFFFFFF);

  static const Color secondaryIconColor = Color(0xFF1E232C);
}

class AppLightColors {
  static const Color primaryBGColor = Color(0xFFFFFFFF);

  static const Color primaryColor = Color(0xff407CE2);
  static const Color primaryLightColor = Color(0xff3498DB);



  static const Color secondaryColor = Color(0xD3F5E0D7);
  static const Color secondaryAltColor = Color(0xFF6A707C);

  static const Color titleTextColor = Color(0xFF1E232C);
  static const Color mainTextColor = Color(0xFF6A707C);
  static const Color mainTextAltColor = Color(0xFFFFFFFF);

  static const Color mainIconColor = Color(0xFFFFFFFF);
  static const Color secondaryIconColor = Color(0xFF1E232C);

  static const Color tabColor = Color(0xfff9f9f9 );   // Dark Slate Blue
  static const Color bgColor = Color(0xffECF0F1);    // Light Gray
  static const Color buttonColor = Color(0xff407CE2); // Bright Red for primary buttons
  static const Color button2Color = Color(0xff3498DB); // Vivid Blue for secondary buttons
  static const Color buttonTextColor = Color(0xffFFFFFF); // White for text on buttons
  static const Color textFormFieldBorderColor = Color(0xffBDC3C7); // Light Gray for input borders
  static const Color textFormFieldLabelColor = Color(0xff2C3E50); // Dark Slate Blue for labels
  static const Color text1Color = Color(0xff2C3E50);  // Dark Slate Blue for main text
  static const Color text2Color = Color(0xff7F8C8D);  // Medium Gray for secondary text
  static const Color text3Color = Colors.orange;  // Bright Red for accents
  static const Color strokeColor = Color(0xffD5D8DC);
}

class AppDarkColors {
  static const Color primaryBGColor = Color(0xFF3E3E49);

  static const Color primaryColor = Color(0xff407CE2);
  static const Color primaryLightColor = Color(0xff3498DB);




  static const Color secondaryColor = Color(0xFF757575);
  static const Color secondaryAltColor = Color(0xFFE0E0E0);

  static const Color titleTextColor = Color(0xFFE0E0E0);
  static const Color mainTextColor = Color(0xFFB0B0B0);
  static const Color mainTextAltColor = Color(0xFF212121);

  static const Color mainIconColor = Color(0xFF1E232C);
  static const Color secondaryIconColor = Color(0xFFE0E0E0);

  static const Color tabColor = Color(0xfff9f9f9 );   // Dark Slate Blue
  static const Color bgColor = Color(0xffECF0F1);    // Light Gray
  static const Color buttonColor = Color(0xff407CE2); // Bright Red for primary buttons
  static const Color button2Color = Color(0xff3498DB); // Vivid Blue for secondary buttons
  static const Color buttonTextColor = Color(0xffFFFFFF); // White for text on buttons
  static const Color textFormFieldBorderColor = Color(0xffBDC3C7); // Light Gray for input borders
  static const Color textFormFieldLabelColor = Color(0xff2C3E50); // Dark Slate Blue for labels
  static const Color text1Color = Color(0xff2C3E50);  // Dark Slate Blue for main text
  static const Color text2Color = Color(0xff7F8C8D);  // Medium Gray for secondary text
  static const Color text3Color = Colors.orange;  // Bright Red for accents
  static const Color strokeColor = Color(0xffD5D8DC); // Warm Gray for strokes and borders
}